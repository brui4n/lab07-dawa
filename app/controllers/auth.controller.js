  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        roles: authorities,
        accessToken: token,
        refreshToken: refreshToken // agregamos refresh token
      }
    });
...
export const refreshToken = async (req, res) => {
  ...
  if (!requestToken) {
    return errorResponse(res, 400, "Refresh Token es requerido");
  }
  ...
  try {
    ...
    if (!refreshToken) {
      return errorResponse(res, 403, "Refresh Token no válido o no encontrado");
    }
    ...
    return errorResponse(res, 500, "Error al renovar token", err.message);
  }
};
...
export const logout = async (req, res) => {
  ...
  try {
    const deleted = await db.refreshToken.destroy({ where: { token: requestToken } });
    ...
    return res.status(200).json({
      success: true,
      message: "Sesión cerrada correctamente (Refresh Token eliminado)"
    });
  } catch (err) {
    return errorResponse(res, 500, "Error al cerrar sesión", err.message);
  }
};
